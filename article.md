...
**Many particles, still two signatures.** The count is signers per fact, not particles on the table, and it is easy to mix the two up. Three, four or five entangled particles share one table, and the toy settles it one pairwise fact at a time: the first settlement narrows the table, the next signs the narrowed table, and the product of the steps is the quantum answer for the whole set, in whatever order the particles settle. We counted that as well, on tables of up to five particles, and it sits on the key. A detector made of a mountain of particles adds links to that chain, never signatures to a fact. So the postulate is sharper than a number: every fact is written in exactly two books, the recorder's and the recorded's, and no fact is ever born in three books at once.

## The run

The whole machine is one small file, and it sits four exams: the Born ladder above, a Bell exam, in which the table after a meeting reaches the quantum value that no product table can, a third exam in which a detector meets one particle of an entangled pair and the odds of the other particle do not move at all, what moves is whom it agrees with, and a fourth in which many entangled particles settle one pair at a time and the whole set lands on the quantum answer. Every step of this page, with the numbers, the worked tables and the four exams, is on [the appendix page](https://questforentropy.com/p/two-to-sign-is-the-norm/mechanism).
