...
**Many particles.** The count is signers per fact, not particles on the table, and the two are easy to mix up. Three, four or five entangled threads share one table, and the toy settles it one pairwise fact at a time: the first settlement narrows the table, the next signs the narrowed table, and the product of the steps is the square of the arrow for the whole joint line, in whichever order the threads settle. That is the chain rule of probability, nothing deeper: the first fact carries the marginal, each later fact the conditional on what is already posted. The fourth exam counts it on GHZ, W and random tables of up to five threads. The alternative a reader will worry about, one fact on three threads signed by all three books at once, is the ladder's third rung again: cubes, and off the key. One catch worth knowing: GHZ and W tables cannot tell the rungs apart, because their non-zero lengths are all equal and every exponent gives the same odds; it takes a table with unequal lengths, a random one, to see the cube fail. A spectator whose ruler does not depend on the line, a thread off the table or in a product state with it, multiplies every line's odds by the same constant and drops out in the normalisation. So a detector made of a mountain of particles adds links to the chain, never signatures to a fact. And a theorem stands behind never needing a three-thread meeting: any turn on any number of threads decomposes into two-thread meetings and one-thread turns, so pairwise meetings reach every evolution the textbooks allow.
...
The whole machine is one small file, and it sits four exams: one axis, two, three, and many.
...
**Many axes, the chain.** GHZ, W and random tables of three, four and five threads, six thousand runs each, settled one thread at a time, each with its own counterparty, and the joint line counted against the square of the arrow. Every one lands within counting noise, at most 0.005 rms, in either order of settlement, at under two ticks per fact. The same random three-thread table with one fact signed by all three books at once misses the key by 0.042, as the ladder says a cube should. And the ladder itself, exact rather than counted, goes on past three: at the 30-degree dial, against a key of 0.750, one to eight books give 0.634, 0.750, 0.839, 0.900, 0.940, 0.964, 0.979, 0.988. More books is sharper than the key and slower to settle; the count is pinned from both sides.
